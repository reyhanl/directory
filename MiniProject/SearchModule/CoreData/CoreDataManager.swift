...
class CoreDataHelper: CoreDataHelperProtocol {
    ...
    func saveNewData<T: Encodable>(entity name: EntityName, object: T) -> Result<Bool, Error> {
        guard let context = stack?.context
        else{
            return .failure(CustomError.somethingWentWrong)
        }
    ...
    func fetchItems(entity name: EntityName, with predicate: NSPredicate?) -> [NSManagedObject] {
        guard let context = stack?.context else{
            return []
        }
        let request = NSFetchRequest<NSFetchRequestResult>(entityName: name.rawValue)
    ...
    func fetchItemsToGeneric<T: Codable>(entity name: EntityName, with predicate: NSPredicate?) -> [T] {
        guard let context = stack?.context else{
            return []
        }
        let request = NSFetchRequest<NSFetchRequestResult>(entityName: name.rawValue)
    ...
    func deleteRecords(entity name: EntityName, with predicate: NSPredicate) -> Result<String, Error> {
        guard let context = stack?.context
        else{
            return .failure(CustomError.somethingWentWrong)
        }
    ...
    func deleteAllRecords(entity name: EntityName) -> Result<String, Error> {
        guard let context = stack?.context
        else{
            return .failure(CustomError.somethingWentWrong)
        }
